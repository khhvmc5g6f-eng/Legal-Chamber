...
This file governs Claude Code sessions editing *this repository's own source*. It is distinct from `skills/legal-work/SKILL.md`, which governs Claude when it is *using* Legal Chambers to do legal work for an end user.
...
3. **Jurisdiction pack changes need a primary source.** Adding or editing anything in `jurisdictions/*/authorities.md`, `*/procedure.md`, or `*/citation.md` requires citing where you got it (official legislation site, official court site, or a named secondary source) and updating that pack's `VERIFICATION_STATUS` honestly, see `docs/SPEC_FULL_TEXT.md` Part CLX and `GOVERNANCE.md`.
...
- Run `scripts/citation_lint.py --selftest`, `scripts/verify_citation_years.py --selftest`, `scripts/style_fix.py --selftest`, `scripts/verify_court_name.py --selftest`, `scripts/deadline_calculator.py --selftest`, `scripts/verify_matter_refs.py --selftest`, `scripts/verify_matter_schema.py --selftest`, `scripts/verify_id_conventions.py --selftest`, `scripts/check_conflicts.py --selftest`, `scripts/verify_matter_persistence.py --selftest`, `scripts/legal_cli.py --selftest`, and `scripts/legal_api.py --selftest` before committing changes that touch citation, style, court-name, deadline, schema cross-reference, schema conformance, ID conventions, conflict checking, matter-persistence, CLI, or API logic.
